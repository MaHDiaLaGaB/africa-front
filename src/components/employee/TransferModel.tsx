// components/employee/TransferModal.tsx
"use client"
...
export default function TransferModal({
  ...
  // 1. Recalculate LYD whenever amount or service changes
  useEffect(() => {
  ...
  // 2. Read from clipboard, send to our API, parse JSON, and fill form
      ...
      <DialogContent className="w-full max-w-full sm:max-w-md p-4 sm:p-6">
        ...
        <div className="space-y-2 sm:space-y-4">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, phoneAccount: e.target.value })}
          />
          <Input
            className="w-full"
            placeholder="البنك/الدولة"
            value={form.bankCountry}
            onChange={(e) => setForm({ ...form, bankCountry: e.target.value })}
          />
          <Input
            className="w-full"
